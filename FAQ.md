...
* Тип вселенной выбирается из 03.03.01.xx
* Если тип вселенной не идентифицирован, тогда в атрибут "Type" вносится запись = `N/A - 'предлагаемый новый тип'`
...
- Агент на основе So that... определяем минимум 5 AC, каждая из которых является практическим доказательством поставляемой ценности или решаемой боли в So that... для I as... в рамках выбранной вселенной. **Кононичный пример** [So that... = Чтобы не болел зуб / Неправильное AC = поставлена пломба / Правильное AC = можно грызть орехи и пить ледяную/горячую воду] -> записывает AC в неудаляемый для L1 атрибут "acceptence_criteria" (далее будет стоять ограничение, что никакую L2 нельзя отдать в составе менее 25% от данного подсчета)
- Агент на основе So that... производит скоринг L2 -> записывает AC в неудаляемый для L2 атрибут "scoring" = (Value*0.6 + (10-Costs)*0.4)/2, где Сosts = (ultra_count*0.6 + complexity*0.2 + connectivity*0.2)  value - это степень достижения ценности So that..., costs - стоимость сбора данных, состоит из (1) "ultra_count" (2) complexity (3) connectivity
* value: (10 - полностью необходим и является ключевым, 0 - не несет смысловой нагрузки, является украшением карты доп. информацией)
* ultra_count: <150 = 10, (125;150] = 9 , (100;125] = 8 , (75;100] = 7 , (50;75] = 6 , (40;50] = 5 , (30;40] = 4 , (20;30] = 3 , (15;20] = 2 , (10;15] = 1 , >10 = 0 
* complexity: (10 - структура запутанна, неоднозначна, противоречива, может быть интерпретирована, 0 - четкая ясная прозрачная структура, разложенная "по полочкам" без двусмысленности)
* connectivity: (10 - каждый элемент порождает кратное увеличие связей в карте, 0 - каждый элемент не привносит дополнительных связей)
- Агент на основе скоринга L2 может не заполнять L2 где "scoring" < 3, но при этом количество заполненных L2 не должно быть менее четырех
- Агент на основе скоринга L2 определяет количественный состав L4 (количество = ultra_count*scoring/10)
...
**Как оценивается:**
- **Высокая Ценность (High Value):** Вектор является ключевым для достижения цели. Без него получение ценности или решение боли по AC и So that... не будет достигнуто
- **Низкая Ценность (Low Value):** Вектор предоставляет вспомогательный, фоновый контекст. Он обогащает картину, но не играет существенной роли в достижении основной ценности по AC и So that...
